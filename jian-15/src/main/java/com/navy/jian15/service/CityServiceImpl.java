package com.navy.jian15.service;

import com.navy.jian15.repository.CityRepository;
...
public class CityServiceImpl implements CityService {
    ...
    public City getPerson(Integer id) {
        log.debug("jian-15 getPerson id is : {}",+ id);
        return cityRepository.getCityById(id);
    }

    @Override
    public List<City> getCity(String id) {
        log.debug("jian-15 getCity id is : {}",id);
    ...
    public City getCityRedis(@RequestParam("cityName") String cityName) {
        log.debug("执行jian-15 getCityRedis cityName is {}",cityName);
        Object result = redisTemplate.opsForValue().get(cityName);
        log.debug("jian-15 getCityRedis result is {}",result);
        return (City)result;
    }
